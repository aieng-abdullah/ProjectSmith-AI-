...
class TestSettings:
    ...
    @patch.dict(os.environ, {'GROQ_API_KEY': 'test-api-key', 'MODEL_NAME': 'test-model', 'TEMPERATURE': '0.5', 'STREAMING': 'false'}, clear=True)
    ...
    @patch('llms.config.load_dotenv')
    @patch.dict(os.environ, {'GROQ_API_KEY': 'test-api-key'}, clear=True)
    def test_load_settings_with_defaults(self, mock_load_dotenv):
        ...
        assert settings.GROQ_API_KEY == 'test-api-key'
        assert settings.MODEL_NAME == 'openai/gpt-oss-120b'  # default
    ...
    @patch('llms.config.load_dotenv')
    @patch.dict(os.environ, {'GROQ_API_KEY': 'test-key', 'STREAMING': 'True'}, clear=True)
    ...
    @patch('llms.config.load_dotenv')
    @patch.dict(os.environ, {'GROQ_API_KEY': 'test-key', 'STREAMING': 'False'}, clear=True)
